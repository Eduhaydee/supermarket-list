...
function renderCart() {
  ...
  cartContainer.innerHTML = cart
    .map((item) => {
      ...
      const discountLabel = `${discount > 0 ? "-" : ""}${Math.abs(
        discount
      ).toFixed(1)}%`;
        ...
        item.name
          ...
          <div class="grid grid-cols-3 gap-2 text-[12px] text-slate-700">
            <div class="flex flex-col gap-1">
              ...
              <span class="text-sm font-semibold text-slate-900">${formatCurrency(
                original
              )}</span>
            </div>
            <label class="flex flex-col gap-1">
              <span class="font-semibold text-slate-700">Qtd</span>
              <input type="number" min="1" step="1" value="${qty}" data-id="${
        item.id
      }" class="qty-input w-full rounded-lg border border-slate-200 px-2 py-2 text-right text-sm shadow-inner">
            </label>
            <label class="flex flex-col gap-1">
              <span class="font-semibold text-slate-700">Pago</span>
              <input type="text" inputmode="decimal" autocomplete="off" value="${formatCurrency(
                paid
              )}" data-id="${
        item.id
      }" class="paid-input currency-input w-full rounded-lg border border-emerald-200 px-2 py-2 text-right text-sm shadow-inner">
            </label>
          </div>
          ...
          <div class="flex items-center justify-between text-sm">
            <div class="flex flex-col gap-1">
              ...
              <span class="${
                discount > 0 ? "text-emerald-600" : "text-slate-700"
              } font-semibold">${discountLabel}</span>
            </div>
            <div class="flex flex-col gap-1 items-end">
              <span class="font-semibold text-slate-700">Total</span>
              <span class="text-sm font-semibold text-emerald-700">${formatCurrency(
                total
              )}</span>
            </div>
          </div>
          ...
          <div class="flex items-center justify-between text-xs text-slate-500">
            <button class="remove-btn font-semibold text-red-500" data-id="${
              item.id
            }">Excluir</button>
  ...
  const totalCart = cart.reduce(
    (sum, item) =>
      sum + (Number(item.paidPrice) || 0) * (Number(item.quantity) || 1),
    0
  );
...
function updatePaid(id, value) {
  ...
  cart = cart.map((item) =>
    item.id === id ? { ...item, paidPrice: paid } : item
  );
  renderCart();
}
...
cartContainer.addEventListener("focusout", (event) => {
  ...
  if (event.target.classList.contains("paid-input")) {
    ...
    const fallbackPaid = item ? Number(item.paidPrice) || Number(item.unitPrice) || 0 : 0;
    const raw = (event.target.value || "").trim();
    const { display, numeric } = raw
      ? parseCurrencyInput(raw)
      : { display: formatCurrency(fallbackPaid), numeric: fallbackPaid };
    event.target.value = display;
  ...
  if (event.target.classList.contains("qty-input")) {
    const item = cart.find((entry) => entry.id === id);
    const fallbackQty = item ? item.quantity || 1 : 1;
    const nextQty = event.target.value === "" ? fallbackQty : event.target.value;
